...
/// Shared options for `cargo build` and `cross build`.
///
/// Hurry options are prefixed with `hurry-` to disambiguate from
/// `cargo`/`cross` args.
...
pub struct HurryBuildOptions {
    ...
    /// These arguments are passed directly to the underlying build command.
    #[arg(
        num_args = ..,
...
impl HurryBuildOptions {
    ...
    #[instrument(name = "HurryBuildOptions::parsed_args")]
