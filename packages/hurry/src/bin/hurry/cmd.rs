...
pub struct HurryOptions {
    ...
    /// Show help for hurry-specific options.
    #[arg(long = "hurry-help", default_value_t = false)]
    pub help: bool,
}

/// Shared options for build commands (`cargo build` and `cross build`).
///
/// This struct contains the common CLI arguments and helper methods
/// used by both cargo and cross build commands.
#[derive(Clone, Args, Debug)]
...
pub struct BuildOptions {
    ...
    /// These arguments are passed directly to the build command as provided.
    #[arg(
        num_args = ..,
...
impl BuildOptions {
    ...
    /// `CargoBuildArguments` for parsing both.
    #[instrument(name = "BuildOptions::parsed_args")]
...
/// Wait for an upload to complete, showing progress.
#[instrument]
