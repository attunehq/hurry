...
use crate::cargo::{cache::WorkspaceCache, invoke, workspace::Workspace};
...
pub fn exec(options: Options) -> Result<()> {
    ...
    let mut workspace_cache = WorkspaceCache::new(&workspace.metadata.workspace_root)
    ...
    let invocation_id = tx
        .query_row(
            ...
            (options.argv.join(" "), OffsetDateTime::now_utc()),
            |row| row.get::<_, i64>(0),
        )
    ...
    // Execute the build.
    match invoke("build", options.argv).context("could not execute build") {
        Ok(()) => {}
        Err(err) => {
